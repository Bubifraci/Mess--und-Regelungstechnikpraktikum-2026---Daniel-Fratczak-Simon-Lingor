...
class CCurve{
  ...
  public:
    ...
    //Funktion um Punkte um Ausgang zu transformieren und zu rotieren
    void transformToPose(double robot_x, double robot_y, double robot_yaw) {
        if (c_err || count <= 0) return;

        double origin_x = tx[0];
        double origin_y = ty[0];

        for (int i = 0; i < count; ++i) {
            double rel_x = tx[i] - origin_x;
            double rel_y = ty[i] - origin_y;

            tx[i] = robot_x + (rel_x * cos(robot_yaw) - rel_y * sin(robot_yaw));
            ty[i] = robot_y + (rel_x * sin(robot_yaw) + rel_y * cos(robot_yaw));
        }
